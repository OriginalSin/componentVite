<script>
	...
	// console.log('tr', gmxMap);
	const addHole = (nm) => {
		console.log('_addHole', nm);
	...
	const notify = (ev) => {
		...
		switch(cmd) {
			...
			case 'addHole': addHole(type, nm); break;
			case 'angle':
			case 'distance':
				_chkDistVal(type, nm, node);
				break;
		}
		// dispatch('notify', ev.detail);
	...
	afterUpdate(() => {
		// console.log('the component just updated', snap, ring);
		redrawItems();
	});
